# compare expression of non significant genes
# between tumor and normal samples in those cohorts
# where the there are at least 20 normal samples
...
# output directory
out_dir <- file.path(
  "/home/ieo5059/mount/local/projects/survival/output/analyses/kaplan-meier/comparison_tumor-normal/quantitative/non-significant", 
  wilcoxon_strategy)
...
gene_comparison_l <- lapply(seq(cohorts), function(j) {
  ...
  non_significant_genes_path <- file.path(
    non_significant_dir,
    cohorts[j], 
    surv_analysis, 
    paste0(
  ...
  if ( file.exists(classification_path) & 
       file.exists(non_significant_genes_path) &
    ...
    non_significant_genes <- read.csv(
      non_significant_genes_path, header = F)[[1]]
    ...
    # loop over genes
    gene_comparison_l <- mclapply(non_significant_genes, function(g) {
      ...
      if ( all(df$exp == 0) ) {
        ...
        # difference in expression between two conditions
